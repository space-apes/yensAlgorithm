...
/*
    ...
    permanent 'committed to' shortest paths (yenPaths) 

    Finally it will return the list of permanent committed to paths. 
    ...
    Function yensKShortestPaths(int K, simpleGraphObject g, String src, String dst)
        ...
        Declare gLocal = {}
        yenPaths[0] = dijkstrasSP(src, dst, g)
        For k = 1 To K
            candidateMinHeap = new minHeap()
            For i = 0 To size(yenPaths[k-1])-2
                spurNode = yenPaths[k-1][i]
                rootPath = yenPaths[k-1].subArray(0,i) //non inclusive on the 2nd value
                deepCopy(g, gLocal)
                For each prevPath in yenPaths
                    if the current rootpath matches the rootPath of prevPath
                        removeEdge(gLocal, prevSpur, prevSpur+1)
                spurPath = dijkStras(gLocal, spurNode, dst)
                totalPath = rootPath+spurPath
                if candidate is not already in yenPaths
                    candidateMinHeap.insert(totalPath)
            End For
            if candidateHeap.heapSize > 0
                yenPaths[k] = candidateMinHeap.getRootNode()
            else
                continue;
        End For
    Return SP
